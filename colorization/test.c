...
void meanOfImages(char* a, char* b, char* c, char* d, char* e, char* f, char* g, char* h, char* i, char* j){
	...
	int cols = pnm_get_width(images[0]);
	int rows = pnm_get_height(images[0]);

	pnm ret = pnm_new(cols, rows, PnmRawPpm);

	unsigned short tmp[10][3];
	for(int i=0; i<cols;i++){
		for (int j = 0; j < rows; j++){
			for(int k=0; k<10;k++){
				tmp[k][0] = pnm_get_component(images[k], j,i,0);
				tmp[k][1] = pnm_get_component(images[k], j,i,1);
				tmp[k][2] = pnm_get_component(images[k], j,i,2);
			}

			unsigned short moy0 = (tmp[0][0]+tmp[1][0]+tmp[2][0]+tmp[3][0]+tmp[4][0]
								  +tmp[5][0]+tmp[6][0]+tmp[7][0]+tmp[8][0]+tmp[9][0])/10;
			unsigned short moy1 = (tmp[0][1]+tmp[1][1]+tmp[2][1]+tmp[3][1]+tmp[4][1]
								  +tmp[5][1]+tmp[6][1]+tmp[7][1]+tmp[8][1]+tmp[9][1])/10;
			unsigned short moy2 = (tmp[0][2]+tmp[1][2]+tmp[2][2]+tmp[3][2]+tmp[4][2]
								  +tmp[5][2]+tmp[6][2]+tmp[7][2]+tmp[8][2]+tmp[9][2])/10;


			pnm_set_component(ret,j,i,0,moy0);
			pnm_set_component(ret,j,i,1,moy1);
			pnm_set_component(ret,j,i,2,moy2);
		}
	}

	pnm_save(ret, PnmRawPpm, "test.ppm");
}
...
int main(int argc, char *argv[]){
	...
	printf("b.ppm   ");
	process(argv[1], argv[2], "b.ppm");
	printf("c.ppm   ");
	process(argv[1], argv[2], "c.ppm");
	printf("d.ppm   ");
	process(argv[1], argv[2], "d.ppm");
	printf("e.ppm   ");
	process(argv[1], argv[2], "e.ppm");
	printf("f.ppm   ");
	process(argv[1], argv[2], "f.ppm");
	printf("g.ppm   ");
	process(argv[1], argv[2], "g.ppm");
	printf("h.ppm   ");
	process(argv[1], argv[2], "h.ppm");
	printf("i.ppm   ");
	process(argv[1], argv[2], "i.ppm");
	printf("j.ppm   ");
	process(argv[1], argv[2], "j.ppm");
	


	meanOfImages("a.ppm", "b.ppm", "c.ppm", "d.ppm", "e.ppm", "f.ppm", "g.ppm", "h.ppm", "i.ppm", "j.ppm");
